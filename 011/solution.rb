...
def products(grid)
  (0...grid.length).flat_map do |x|
    (0...grid[x].length).flat_map do |y|
      [
        product(grid[x][y..y + 3]),
        product(grid[x..x + 3].map { |r| r[y] }),
        product((0..3).map { |i| grid.fetch(x + i) { [] }[y + i] }),
        product((0..3).map { |i| grid.fetch(x + i) { [] }[y - i] })
      ]
    end
  end
